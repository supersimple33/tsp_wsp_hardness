...
def find_subproblems(tree_node, add=True):
    subproblems = []
    ...
    if issubclass(quadtree, ds.AbstractPKQuadTree):
        ...
        if tree_node.divided:
            ne = find_subproblems(tree_node.ne, True)
            nw = find_subproblems(tree_node.nw, True)
            sw = find_subproblems(tree_node.sw, True)
            se = find_subproblems(tree_node.se, True)
            if isinstance(ne, ds.Point) or len(ne) > 0:
                subproblems.append(ne)
            if isinstance(nw, ds.Point) or len(nw) > 0:
                subproblems.append(nw)
            if isinstance(sw, ds.Point) or len(sw) > 0:
                subproblems.append(sw)
            if isinstance(se, ds.Point) or len(se) > 0:
                subproblems.append(se)

    if len(subproblems) == 1:
        subproblems = subproblems[0]
    return subproblems

grouped_points = find_subproblems(wspTreeNode)
...
def find_path(start, glist, end, depth=-1, init=False):
    ...
    if init:
        ...
        print("start", start, "glist", glist)
        if isinstance(glist, list) and start not in glist:
            start_item = find_list_with(start, glist)
        if isinstance(glist, list) and end not in glist:
