...
class TravellingSalesmanProblem(Generic[QuadTreeType]):  # TODO: better use of generics
    ...
    def __init__(
        self,
        quadtree: ds.AbstractQuadTree,
        ax: np.ndarray[Optional[Axes]],
        s=1.0,
        wspd=None,
    ):  # TODO: utilize WSPD?
        """Initializes the TravellingSalesmanProblem, NOTE: takes in an already built quadtree"""
        ...
        if ax[0] is not None:
            ax[0].plot(
                [self.quadtree.boundary.xMin, self.quadtree.boundary.xMax],
                [self.quadtree.boundary.yMin, self.quadtree.boundary.yMin],
                color="gray",
            )
            ax[0].plot(
                [self.quadtree.boundary.xMin, self.quadtree.boundary.xMax],
                [self.quadtree.boundary.yMax, self.quadtree.boundary.yMax],
                color="gray",
            )
            ax[0].plot(
                [self.quadtree.boundary.xMin, self.quadtree.boundary.xMin],
                [self.quadtree.boundary.yMin, self.quadtree.boundary.yMax],
                color="gray",
            )
            ax[0].plot(
                [self.quadtree.boundary.xMax, self.quadtree.boundary.xMax],
                [self.quadtree.boundary.yMin, self.quadtree.boundary.yMax],
                color="gray",
            )
        if ax[1] is not None:
        ...
        if (
            isinstance(quadtree, ds.AbstractPKQuadTree)
            and not self.quadtree.pk_aggregated
        ):
        ...
        if not self.quadtree.path_compressed:
            self.quadtree = (
                self.quadtree.path_compress()
            )  # TODO: Ensure these are inconsequential/not slow
    ...
    def __init__(
        self, treeType, points: list[ds.Point], ax: np.ndarray[Optional[Axes]], s=1.0
    ):
        """Initializes the TravellingSalesmanProblem, NOTE: the base case version which builds its own quadtree"""
        assert (
            s >= 1.0
        ), "Separation factor must be greater than or equal to 1.0"  # make stricter later?
        self._s = s
        ...
        self.points = points
        assert len(self.points) == len(self.quadtree.covered_points) and set(
            self.points
        ) == set(
            self.quadtree.covered_points
        ), "Points and quadtree points do not match"

        self.point_to_ids_map = {point: i for i, point in enumerate(self.points)}

    def __contains__(self, point: ds.Point) -> bool:
    ...
    def draw_tour(
        self, tour: list[ds.Point], color="r", linestyle="-", label=None, linewidth=None
    ):
        ...
        path = patches.PathPatch(
            patches.Path([point.to_tuple() for point in tour]),
            facecolor="none",
            edgecolor=color,
            linestyle=linestyle,
            label=label,
    ...
    def wspd(self) -> set[tuple[frozenset[QuadTreeType, QuadTreeType], ds.SpecialDist]]:
        ...
        def recursive_wspd(
            node_A: QuadTreeType, node_B: QuadTreeType
        ):  # could be stricter with typing
            if (
                len(node_A) == 0
                or len(node_B) == 0
                or (node_A.leaf and node_B.leaf and node_A == node_B)
            ):
                return

            big_radius = max(
                0 if node_A.leaf else node_A.radius, 0 if node_B.leaf else node_B.radius
            )  # REVIEW: or just node_A.radius?
            small_radius = min(
                0 if node_A.leaf else node_A.radius, 0 if node_B.leaf else node_B.radius
            )
            ...
            bubble_dist = (node_A.center - node_B.center).mag() - (
                big_radius + small_radius
            )
            if bubble_dist >= self._s * big_radius:  # node_A guaranteed to be bigger
                special_dist = ds.SpecialDist.from_radius(
                    bubble_dist, self._s * big_radius
                )
                ws_pairs.add((frozenset((node_A, node_B)), special_dist))
                return
            # else:
            #     pass # ??
            if (float("-inf") if node_A.leaf else node_A.radius) < (
                float("-inf") if node_B.leaf else node_B.radius
            ):  # REVIEW: correct comparator?
    ...
    def draw_wsp_pair(
        ...
        adjust=0.02,
        linewidth=1.0,
    ):  # pretty sure the kwargs are safe to be extracted
        """Draws a single WSP pair on the matplotlib axes"""
        point_A = (
            node_A.boundary.center()
            if use_boundary
            else (node_A.points[0] if node_A.leaf else node_A.center)
        )  # REVIEW: points[0] or mean point?
        point_B = (
            node_B.boundary.center()
            if use_boundary
            else (node_B.points[0] if node_B.leaf else node_B.center)
        )
            ...
            patches.Path(
                [
                    ...
                    (
                        midpoint.x + nudge,
                        midpoint.y + nudge,
                    ),  # helpful in big picture mode to add curvature
                    (point_B.x, point_B.y),
                ]
            ),
            linestyle=ls,
        ...
        # draw the circles
        if not no_circles:
            big_radius = max(node_A.radius, node_B.radius) * (
                1 if node_A.leaf and node_B.leaf else 1
            )
            ...
            self.ax[0].add_artist(
                circle1
            )  # calling add_artist instead of add_patch will cut off big circles
            self.ax[0].add_artist(
                circle2
            )  # this is a god enough fix for now and circles still end up in patches
    ...
    def draw_wspd(
        ...
        for (node_A, node_B), _ in self.wspd:
            self.draw_wsp_pair(
                node_A, node_B, no_leaves, use_boundary, no_circles, adjust, linewidth
            )
    ...
    def on_click(
        ...
        if event.button == 1:  # cycle
            ...
            while (
                no_leaves and node_A.leaf and node_B.leaf
            ):  # DANGER: infinite loop if only leaves
                self.current_screen = (self.current_screen + 1) % len(self.wspd)
                node_A, node_B, _ = self.wspd[self.current_screen]
            self.ax[0].set_title(
                f"Showing WSP#{self.current_screen + 1}/{len(self.wspd)}"
            )
            self.draw_wsp_pair(
                node_A, node_B, no_leaves, use_boundary, no_circles, adjust, linewidth
            )
    ...
    def brute_force_path(self) -> tuple[list[ds.Point], float, tuple]:
        ...
        for perm in perms:
            num_perms += 1
            dist = (
                self.dist_matrix[0][self.point_to_ids_map[perm[0]]]
                + self.calc_dist(perm)
                + self.dist_matrix[0][self.point_to_ids_map[perm[-1]]]
            )
            if dist < min_dist:
    ...
    def dp_path(self) -> tuple[list[ds.Point], float, tuple]:
        ...
        parent = np.full(
            (2**n, n), -1
        )  # record the parent node of each path so we can make later

        arr[0][0] = 0
        for i in range(1, n):
            arr[2**i][i] = self.dist_matrix[0][i]
        ...
        for mask in range(2**n):
            for i in range(n):  # TODO: check out that itertools.product thing
                if arr[mask][i] == float(
                    "inf"
                ):  # ensure that the path being travelled makes sense
                    continue
                for j in range(n):
                    perms += 1
                    if mask & (2**j) != 0:  # ensure we are travelling to a new node
                        continue
                    new_mask = mask | (2**j)
                    if arr[new_mask][j] > arr[mask][i] + self.dist_matrix[i][j]:
                        arr[new_mask][j] = arr[mask][i] + self.dist_matrix[i][j]
                        parent[new_mask][j] = i
    ...
    def dp_alt_path(self) -> tuple[list[ds.Point], float, tuple]:
        ...
        assert np.isclose(
            dist, c_dist
        ), f"Distance {c_dist} does not match calculated distance {dist}"
    ...
    def bab_path(self) -> tuple[list[ds.Point], float, tuple]:
        ...
        assert np.isclose(
            dist, c_dist
        ), f"Branch and bound distance {c_dist} does not match calculated distance {dist}"
    ...
    def ishan_bfp_path(self) -> tuple[list[ds.Point], float, tuple]:
        ...
        def buildPerms(perm, rem):
            ...
            for r in rem:
                ...
                if (
                    r in ws[last_point]
                ):  # checks if all points in last_point <-> r set have been visited
    ...
    def generate_sub_problem_order(self, start, t) -> list[QuadTreeType]:
        ...
        sub_problem_order: list[QuadTreeType] = (
            []
        )  # order in which to visit subproblems
        ...
        def try_to_add(subtree: QuadTreeType) -> bool:
            ...
            if len(subtree) <= t:
                ...
                sub_tsp = TravellingSalesmanProblem(
                    subtree, np.array([None, None]), self._s, None
                )
                sub_sub_order = sub_tsp.generate_sub_problem_order(None, t)
                sub_problem_order.extend(sub_sub_order)
                collected_points.update(
                    subtree.covered_points
        ...
        assert try_to_add(
            current_subtree
        ), "Should be guaranteed to insert the first one"
        while len(collected_points) < len(self.points):
            for problem in self.single_indexable_wspd[current_subtree]:
                ...
                jumpable = min(
                    filter(
                        lambda a: all(
                            point not in collected_points for point in a.covered_points
                        ),
                        self.single_indexable_wspd.keys(),
                    ),
                    key=lambda b: (b.center - current_subtree.center).mag(),
                )
                assert try_to_add(
                    jumpable
                ), "Should have selected a guaranteed jumpable node"
    ...
    def nwsp_path(self, t=8) -> tuple[list[ds.Point], float, tuple]:
        ...
        start, entry_point = util.min_proj(
            sub_problem_order[0].covered_points, sub_problem_order[1].covered_points
        )
        tour.append(start)
        for i in range(1, len(sub_problem_order) - 1):
            exit_point, next_entry = util.min_proj(
                sub_problem_order[i].covered_points,
                sub_problem_order[i + 1].covered_points,
            )
            ...
            ):  # entry and exit may not be equal, TODO: make this better
                popped = tuple(
                    filter(
                        lambda x: x != entry_point, sub_problem_order[i].covered_points
                    )
                )
                exit_point, next_entry = util.min_proj(
                    popped, sub_problem_order[i + 1].covered_points
                )
            ...
            tour.extend(
                util.hamiltonian_path(
                    entry_point, exit_point, sub_problem_order[i].covered_points
                )
            )
            entry_point = next_entry
        ...
        if sub_problem_order[-1].leaf and sub_problem_order[0].leaf:
            ...
            tour.extend(
                util.hamiltonian_path(
                    entry_point,
                    start,
                    sub_problem_order[0].covered_points + (entry_point,),
                )
            )
            return tour, self.calc_dist(tour), None  # TODO: fill in None
            ...
            tour.extend(
                util.hamiltonian_path(
                    entry_point, start, sub_problem_order[-1].covered_points + (start,)
                )
            )
            return tour, self.calc_dist(tour), None  # TODO: fill in None

        exit_point, next_entry = util.min_proj(
            sub_problem_order[-1].covered_points, sub_problem_order[0].covered_points
        )
        if (
            entry_point == exit_point or next_entry == start
        ):  # entry and exit may not be equal, TODO: make this better
            popped_end = (
                tuple(
                    filter(
                        lambda x: x != entry_point, sub_problem_order[-1].covered_points
                    )
                )
                if entry_point == exit_point
                else sub_problem_order[-1].covered_points
            )
            popped_start = (
                tuple(filter(lambda x: x != start, sub_problem_order[0].covered_points))
                if next_entry == start
                else sub_problem_order[0].covered_points
            )
            exit_point, next_entry = util.min_proj(popped_end, popped_start)
        tour.extend(
            util.hamiltonian_path(
                entry_point, exit_point, sub_problem_order[-1].covered_points
            )
        )
        tour.extend(
            util.hamiltonian_path(
                next_entry, start, sub_problem_order[0].covered_points
            )
        )
    ...
    def draw_ordering(self, sub_problem_order: list[QuadTreeType]) -> None:
        ...
        for i, node in enumerate(sub_problem_order):
            ...
            self.ax[1].fill_between(
                [node.boundary.xMin, node.boundary.xMax],
                [node.boundary.yMin, node.boundary.yMin],
                [node.boundary.yMax, node.boundary.yMax],
                color=color_map(i),
                alpha=0.5,
            )
    ...
    def spop_path(
        self,
    ) -> tuple[list[ds.Point], float, tuple]:  # TODO: extract into new file
        pass
    ...
    def number_seed(self):
        return (
            len(self.points)
            + min([p.x * 100 for p in self.points])
    ...
    def local_search_path(self, path=None) -> tuple[list[ds.Point], float, int]:
        ...
        id_path, c_dist = solve_tsp_local_search(
            self.dist_matrix,
            self.point_tour_to_ids(path[:-1]),
            perturbation_scheme="ps6",
            rng=Random(self.number_seed),
        )
        ...
        assert np.isclose(
            dist, c_dist
        ), f"Local Search distance {c_dist} does not match calculated distance {dist}"
    ...
    def quick_local_search_path(self) -> tuple[list[ds.Point], float, int]:
        ...
        iters = int(
            0.1 * 4.65 * (len(self.points) ** 2)
        )  # NOTE: a rough estimate of 10% of the average number of iterations
        id_path, c_dist = solve_tsp_local_search(
            self.dist_matrix,
            self.point_tour_to_ids(self.nnn_path[0][:-1]),
        ...
        assert np.isclose(
            dist, c_dist
        ), f"Quick Local Search distance {c_dist} does not match calculated distance {dist}"
    ...
    def simulated_annealing_path(self) -> tuple[list[ds.Point], float, None]:
        ...
        id_path, c_dist = solve_tsp_simulated_annealing(
            self.dist_matrix,
            self.point_tour_to_ids(self.nnn_path[0][:-1]),
            perturbation_scheme="two_opt",
            alpha=0.9,
    ...
    def lkh_path(self) -> tuple[list[ds.Point], float, None]:
        ...
        id_path, c_dist = solve_tsp_lin_kernighan(
            self.dist_matrix, self.point_tour_to_ids(self.nnn_path[0][:-1])
        )
    ...
    def rtr_path(self, path=None, seed=None) -> tuple[list[ds.Point], float, None]:
        ...
        id_path, c_dist = solve_tsp_record_to_record(
            self.dist_matrix, self.point_tour_to_ids(path[:-1]), rng=Random(seed)
        )
    ...
    def check_tour(self, path: list[ds.Point]) -> bool:
        ...
        return (
            path[0] == path[-1]
            and (len(path) == len(self.points) + 1)
    ...
    def num_jumps(path, ideal) -> int:
        ...
        path_connections = {
            frozenset((path[i], path[0 if len(path) - 1 == i else i + 1]))
            for i in range(len(path))
        }
        ideal_connections = {
            frozenset((ideal[i], ideal[0 if len(ideal) - 1 == i else i + 1]))
