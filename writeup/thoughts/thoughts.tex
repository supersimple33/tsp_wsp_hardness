...
\begin{proof}
    ...
    There must exist at least one permutation of deletions such that the resulting graphcontains no cycles.
    This can be shown by construction.
    Consider some $P^*$ which contains at least one disjoint cycle and a path $R$ from $a'$ to $a''$ which contains at least one edge in $E$.\footnote{This can be formed by choosing any permutation which include the first edge in $E$ which is encountered in the path from $a'$ to $a''$.}
    Since there is at least one cycle $R$ must not contain every point in $P$.
    We can then traverse $R$ from $a'$ to $a''$ and note the last edge from $E$ which is encountered.
    If this edge was of the form $e_i$ then we can delete it and replace it with $e'_i$ and if it was of the form $e'_i$ then we can delete it and replace it with $e_i$.
    This forms a new valid permutation.
    The new path $R'$ from $a'$ to $a''$ of this new permutation must contain strictly more new intra-set edges of the form $e^*_i$ than $R$ did since the last edge from $E$ in $R$ was replaced with an intra-set edge of the form $e^*_i$.
    We can then repeat this process until we have a path from $a'$ to $a''$ which contains all of the edges of the form $e^*_i$ and thus contains no cycles and thus is a valid Hamiltonian path over $P$.
    Let this new Hamiltonian path be $H^*$.


    Let $e_i^{kept}$ be the edge from the pair $(e_i, e'_i)$ which was kept in $H^*$ and let $e_i^{deleted}$ be the edge from the pair $(e_i, e'_i)$ which was deleted in $H^*$.
