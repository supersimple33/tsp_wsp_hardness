...
\usepackage[perpage,bottom,multiple,hang]{footmisc}
...
\newcommand{\ACollection}{\mathcal{A}}
...
\input{../MSThesis/theorems/wsp_mtour_mixed.tex}
...
\input{../MSThesis/theorems/wsp_mpath_cycle_preserve_no_mix.tex}
% corollary if all endpoint in the same set then these two edges must be on the same path

\input{../MSThesis/theorems/wsp_mpath_cycle_preserve_even_mix.tex}

\input{../MSThesis/theorems/wsp_mpath_cycle_preserve_odd_mix.tex}
...
\begin{defn}\label{defn:set_transition_multidigraph}
    Let $P = \bigcup_i A_i$ be a set of points where the elements of $\ACollection = \{A_1, A_2, \ldots, A_n\}$ are all pairwise $s$-well-separated with $s \geq 1$ (and $n \geq 2$).
    We define the unique \textbf{Set Transition Multidigraph} $G = (V,E)$ induced by some Hamiltonian path over $P$ to be the simple (eulerian or semi-eulerian)multidigraph where $V = \{A_1, A_2, \ldots, A_n\}$ and there is an edge from $A_i$ to $A_j$ for every time the Hamiltonian path over $P$ transitions from a point in $A_i$ and to a point in $A_j$ for $i \neq j$.
    Futhermore, we define the unique \textbf{Induced Eulerian Walk} to be the walk through $G$ which follows the edges of $G$ in the order that they are traversed by the Hamiltonian path over $P$ as it transitions between the sets in $\ACollection$.
\end{defn}

\input{../MSThesis/theorems/direct_k_wsp_path.tex}

\begin{corollary}\label{cor:set_transition_multidigraph_reduction}
    Let $P = \bigcup_i A_i$ be a set of points where the elements of $\ACollection = \{A_1, A_2, \ldots, A_n\}$ are all pairwise $s$-well-separated with $s \geq 1$ (and $n \geq 2$).
    Then there exists an optimal Hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$ such that the set transition multidigraph  induced by the path reduces to a simple digraph (i.e. there are no parallel directed edges).
    If $s > 1$ then all optimal Hamiltonian paths must satisfy this property.
\end{corollary}

\begin{lem}\label{lem:set_transition_multidigraph_decomposition}
    Let $P = \bigcup_i A_i$ be a set of points where the elements of $\ACollection = \{A_1, A_2, \ldots, A_n\}$ are all pairwise $s$-well-separated with $s \geq 1$ (and $n \geq 2$).
    Then there exists an optimal Hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$ such that the set transition multidigraph  induced by the path can be decomposed into a set of edge-disjoint simple directed cycles and if $A_i \neq A_j$ then a single directed simple path from $A_i$ to $A_j$ also edge-disjoint from the simple cycles.
    If $s > 1$ then all optimal Hamiltonian paths must satisfy this property.
\end{lem}
\begin{proof}
    Follows from \Cref{defn:set_transition_multidigraph,cor:set_transition_multidigraph_reduction}.
    %See Veblen's Theorem \autocite{veblenApplicationModularEquations1912}. TODO
\end{proof}

\subsubsection{Triple Pairwise Separation}

\begin{thm}\label{thm:triple_wsp_path_ac}
    Let $P = A \cup B \cup C$ be a set of points where $A,B,C$ are pairwise $s$-well-separated with $s \geq 1.5$. 
    Then there exists an optimal Hamiltonian path which starts on a point $a' \in A$ and ends on a point $c' \in C$ and must have only two or three edges connecting the three sets.
    Furthermore if $s > 1.5$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
    Consider some optimal Hamiltonian path over $P$ with endpoints $a' \in A$ and $c' \in C$.
    It follows from \Cref{cor:set_transition_multidigraph_reduction} that the set transition multidigraph induced by this path can be reduced to a simple digraph.

    A simple digraph over three nodes can have at most six edges.
    If the digraph has six edges then all three nodes are balanced and thus could not be a semi-Eulerian digraph as required.
    It can also be noted that $B$ must be balanced and therefore must have $deg^+(B) = deg^-(B) \in \{0,1,2\}$.

    \textbf{Case 1:} $\deg^+(B) = \deg^-(B) = 2$. % ab ba bc cb ac   ab bc ca ac
    Then there are one edge in each direction between $A$ and $B$ and one edge in each direction between $B$ and $C$.
    Then the fifth edge must be from $A$ to $C$.
    It subsequently follows that the following three cases are the only valid eulerian walks through the digraph:
    \begin{enumerate}
        \item $A \to B \to C \to B \to A \to C$
        \item $A \to B \to A \to C \to B \to C$
        \item $A \to C \to B \to A \to B \to C$
    \end{enumerate}
    We can then consider each edge from the perspective of the Hamiltonian path over $P$ and label them as follows $(a_1, b_1), (b_2, a_2), (c_1, b_3), (b_4, c_2), (a_3, c_3)$ where $a_i \in A$, $b_i \in B$, and $c_i \in C$.
    
    \textbf{Case 1a:} The first walk then corresponds to the following Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to b_1 \leadsto b_4 \to c_2 \leadsto c_1 \to b_3 \leadsto b_2 \to a_2 \leadsto a_3 \to c_3 \leadsto c'$$
    We could then consider a new Hamiltonian path over $P$ where we cut the edges $(a_1, b_1), (c_1, b_3), (a_3, c_3)$ and replace them with $(a_1, a_3), (b_3, b_1), (c_1, c_3)$ resulting in a new Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to a_3 \leadsto a_2 \to b_2 \leadsto b_3 \to b_1 \leadsto b_4 \to c_2 \leadsto c_1 \to c_3 \leadsto c'$$

    \textbf{Case 1b:} The second walk then corresponds to the following Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to b_1 \leadsto b_2 \to a_2 \leadsto a_3 \to c_3 \leadsto c_1 \to b_3 \leadsto b_4 \to c_2 \leadsto c'$$
    We could then consider a new Hamiltonian path over $P$ where we cut the edges $(a_1, b_1), (a_3, c_3), (b_4, c_2)$ and replace them with $(a_1, a_3), (b_1, b_4), (c_3, c_2)$ resulting in a new Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to a_3 \leadsto a_2 \to b_2 \leadsto b_1 \to b_4 \leadsto b_3 \to c_1 \leadsto c_3 \to c_2 \leadsto c'$$

    \textbf{Case 1c:} The third walk then corresponds to the following Hamiltonian path over $P$:
    $$a' \leadsto a_3 \to c_3 \leadsto c_1 \to b_3 \leadsto b_2 \to a_2 \leadsto a_1 \to b_1 \leadsto b_4 \to c_2 \leadsto c'$$
    We could then consider a new Hamiltonian path over $P$ where we cut the edges $(a_3, c_3), (b_2, a_2), (b_4, c_2)$ and replace them with $(a_3, a_2), (b_4, b_2), (c_3, c_2)$ resulting in a new Hamiltonian path over $P$:
    $$a' \leadsto a_3 \to a_2 \leadsto a_1 \to b_1 \leadsto b_4 \to b_2 \leadsto b_3 \to c_1 \leadsto c_3 \to c_2 \leadsto c'$$

    Then by \Cref{thm:fundamental_thm_wsp} it follows that across each case the current Hamiltonian path over $P$ could not possibly be optimal.\footnote{
        Assuming that the original Hamiltonian path was optimal each case implies that $s \leq 1$ which is a contradiction.
        \begin{enumerate}
            \item $|a_1 b_1| + |c_1 b_3| + |a_3 c_3| \leq |a_1 a_3| + |b_3 b_1| + |c_1 c_3| \leq \frac{1}{s} (|a_1 b_1| + |c_1 b_3| + |a_3 c_3|)$
            \item $|a_1 b_1| + |a_3 c_3| + |b_4 c_2| \leq |a_1 a_3| + |b_1 b_4| + |c_3 c_2| \leq \frac{1}{s} (|a_1 b_1| + |a_3 c_3| + |b_4 c_2|)$
            \item $|a_3 c_3| + |b_2 a_2| + |b_4 c_2| \leq |a_3 a_2| + |b_4 b_2| + |c_3 c_2| \leq \frac{1}{s} (|a_3 c_3| + |b_2 a_2| + |b_4 c_2|)$
        \end{enumerate}
    }

    \textbf{Case 2:} $\deg^+(B) = \deg^-(B) = 1$. % ab ba ac
    There are only two valid simple digraphs under this case which have four edges: $(A,B), (B,C), (C,A), (A,C)$ and $(A,C), (C,A), (A,B), (B,C)$.
    
    \textbf{Case 2a:} The first digraph corresponds to the following Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to b_1 \leadsto b_2 \to c_1 \leadsto c_2 \to a_2 \leadsto a_3 \to c_3 \leadsto c'$$
    We could then consider a new Hamiltonian path over $P$ where we cut the edges $(c_2, a_2), (a_3, c_3), (a_1, b_1)$ and replace them with $(a_1, a_2), (a_3, b_1), (c_2, c_3)$ resulting in a new Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to a_2 \leadsto a_3 \to b_1 \leadsto b_2 \to c_1 \leadsto c_2 \to c_3 \leadsto c'$$
    It then follows from \Cref{thm:fundamental_thm_wsp} that if $s = 1.5$ then the new Hamiltonian path is also optimal and if $s > 1.5$ then the original Hamiltonian path could not possibly be optimal as that would require $s \leq 1.5$.\footnote{Consider the following analysis:
        \begin{align*}
            |c_2 a_2| + |a_3 c_3| + |a_1 b_1| &\leq |a_1 a_2| + |a_3 b_1| + |c_2 c_3| \\
            &\leq |a_1 a_2| + |a_3 a_1| + |a_1 b_1| + |c_2 c_3| \\
            \implies |c_2 a_2| + |a_3 c_3| &\leq |a_1 a_2| + |a_3 a_1| + 0.5 |c_2 c_3| + 0.5 |c_2 c_3| \\
            &\leq \frac{1}{s} |c_2 a_2| + \frac{1}{s} |a_3 c_3| + \frac{1}{2s} |c_2 c_3| + \frac{1}{2s} |c_2 c_3| \\
            &= \frac{1.5}{s} (|c_2 a_2| + |a_3 c_3| + |c_2 c_3|) \\
            \implies s &\leq 1.5
        \end{align*}
    }

    \textbf{Case 2b:} The second digraph corresponds to the following Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to c_1 \leadsto c_2 \to a_2 \leadsto a_3 \to b_1 \leadsto b_2 \to c_3 \leadsto c'$$
    We could then consider a new Hamiltonian path over $P$ where we cut the edges $(a_1, c_1), (c_2, a_2), (b_2, c_3)$ and replace them with $(a_1, a_2), (b_2, c_1), (c_2, c_3)$ resulting in a new Hamiltonian path over $P$:
    $$a' \leadsto a_1 \to a_2 \leadsto a_3 \to b_1 \leadsto b_2 \to c_1 \leadsto c_2 \to c_3 \leadsto c'$$
    It then follows from \Cref{thm:fundamental_thm_wsp} that if $s = 1.5$ then the new Hamiltonian path is also optimal and if $s > 1.5$ then the original Hamiltonian path could not possibly be optimal as that would require $s \leq 1.5$.\footnote{Consider the following analysis:
        \begin{align*}
            |a_1 c_1| + |c_2 a_2| + |b_2 c_3| &\leq |a_1 a_2| + |b_2 c_1| + |c_2 c_3| \\
            &\leq |a_1 a_2| + |b_2 c_3| + |c_3 c_1| + |c_2 c_3| \\
            \implies |a_1 c_1| + |c_2 a_2| &\leq |a_1 a_2| + |c_3 c_1| + |c_2 c_3| \\
            &= 0.5 |a_1 a_2| + 0.5 |a_1 a_2| + |c_3 c_1| + |c_2 c_3| \\
            &\leq \frac{1}{2s} |a_1 c_1| + \frac{1}{2s} |c_2 a_2| + \frac{1}{s} |a_1 c_1| + \frac{1}{s} |c_2 a_2| \\
            &= \frac{1.5}{s} (|a_1 c_1| + |c_2 a_2|) \\
            \implies s &\leq 1.5
        \end{align*}
    }

    It therefore follows that if $s \geq 1.5$ then there exists an optimal Hamiltonian path which starts on a point $a' \in A$ and ends on a point $c' \in C$ and must have only two or three edges connecting the three sets.
    Futhermore if $s > 1.5$ then all optimal solutions must satisfy this property.
\end{proof}
...
\begin{thm}
    Let $P = \bigcup_{i=1}^n A_i$ be a set of points where $A_1, A_2, \ldots, A_n$ are all pairwise $s$-well-separated with $s \geq 1.0$ (and $n \geq 3$).
    Then there exists an optimal Hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$ where its induced set transition multidigraph $G$ never contains two directed simple cycles which are symmetric with respect to each other.
    Furthermore if $s > 1.0$ then all optimal Hamiltonian paths must satisfy this property.
\end{thm}
\begin{proof}
    Consider some optimal Hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$.
    Suppose that the set transition digraph $G$ induced by this path contains two directed cycles which are symmetric with respect to each other.

    Consider two symmetric cycles label one $C_1$ and the other $C_2$.
    Let the sequence of sets traversed in $C_1$ be $A_{k_1} \to A_{k_2} \to \dots \to A_{k_m} \to A_{k_1}$.
    By symmetry, $C_2$ traverses in reverse order: $A_{k_1} \to A_{k_m} \to \dots \to A_{k_2} \to A_{k_1}$.

    Let $E_1 = \{e_1, e_2, \dots, e_m\}$ be the specific edges in the Hamiltonian path that induce the cycle $C_1$, where each $e_i$ corresponds to a transition from a point in $A_{k_i}$ to a point in $A_{k_{i+1}}$ (with indices modulo $m$).
    Similarly, let $E_2 = \{e'_1, e'_2, \dots, e'_m\}$ be the edges inducing $C_2$, where each $e'_i$ corresponds to a transition from a point in $A_{k_{i+1}}$ to a point in $A_{k_i}$.
    We can define a combination of the two sets $E = \{\{e_1, e'_1\}, \{e_2, e'_2\}, \dots, \{e_m, e'_m\}\}$.
    Next we will consider removing any permutation of $m$ edges from the Hamiltonian path where we remove exactly one edge from each pair in $E$.
    This will break the Hamiltonia path into $m+1$ disjoint subpaths.
    We can then replace the deleted edge from each $i$th pair with a new set-internal edge which connects $u$ and $z$ where $u$ is the endpoint of the deleted edge in the $i$th pair which belongs to $A_{k_i}$ and $z$ is the endpoint of the deleted edge in the $i-1$th pair which belongs to $A_{k_i}$ we will label this new edge as $e^*_i$.
    
    Let $\GraphSet$ be the set of all $2^m$ possible permutations.
    It can be noted that for every graph in $\GraphSet$, the degree of every vertex is exactly two except for the endpoints $a'$ and $a''$ which have degree one (the same as the original Hamiltonian path).
    It subsequently follows that each graph in $\GraphSet$ is formed of a single path $R$ from $a'$ to $a''$ and $c \geq 1$ cycles (all disjoint from each other and the path).
    There must exist at least one permutation of deletions such that the resulting graph contains no cycles (it is a Hamiltonian path).
    This can be shown by construction.
    Let $G_{old}$ be a graph in $\GraphSet$ which contains at least one cycle.

    Define $\hat{e_i}$ to be the edge from the pair $(e_i, e'_i)$ which is present in $G_{old}$ and $\tilde{e_i}$ to be the edge from the pair $(e_i, e'_i)$ which is not present in $G_{old}$.
    Note that $R$ must contain at least one intra-set edge of the form $e^*_i$ for some $i \in \{1, \dots, m\}$ since it would otherwise be impossible to connect the endpoints $a'$ and $a''$ and also must not contain every intra-set edge of the form $e^*_i$ for some $i \in \{1, \dots, m\}$ since it would otherwise be impossible to have any cycles in the graph.
    Thus there must exist at least one pair $(e^*_i, e^*_{i+1})$ such that $e^*_i \in R$ and $e^*_{i+1} \notin R$.
    Let $Z$ be the distinct cycle in $G_{old}$ which contains $e^*_{i+1}$.
    Let $Y$ be the distinct component in $G_{old}$ which contains $\hat{e_i}$.
    Then we can consider creating a new graph $G_{new} \in \GraphSet$ where we swap the deleted and kept edges in the $i$th inter-set pair such that $\hat{e_i}$ is deleted and $\tilde{e_i}$ is kept.
    
    Let $a,b \in A_{k_i}$ be the endpoints of $e^*_i$ and let $c,d \in A_{k_{i+1}}$ be the endpoints of $e^*_{i+1}$.
    Furthermore let $u \in A_{k_i}$ and $v \in A_{k_{i+1}}$ be the endpoints of $\hat{e_i}$.
    Finally let $b,d$ be the endpoints of $\tilde{e_i}$ (this is the case as one endpoint from $e^*_i$ and one endpoint from $e^*_{i+1}$ must be used to form $\tilde{e_i}$).
    Then it follows that $G_{new}$ is formed by removing the edges $(a,b),(c,d),(u,v)$ and adding the edges $(b,d),(a,u),(c,v)$.

    After the deletions the affected components $\{R, Z, Y\}$ are partitioned into exactly four disjoint connected subpaths, which we denote $\PathSet = \{P_1, P_2, P_3, P_4\}$. 
    This count of four subpaths holds universally regardless of whether $Y = R$, $Y = Z$, or all three components are distinct:
    \begin{itemize}
        \item \textbf{If $Y \neq R$ and $Y \neq Z$:} Deleting one edge from $R$ yields $2$ subpaths, while deleting one edge from each cycle $Z$ and $Y$ opens each into $1$ subpath.
        \item \textbf{If $Y = R$:} Deleting two edges from $R$ yields $3$ subpaths, while deleting one edge from $Z$ opens it into $1$ subpath.
        \item \textbf{If $Y = Z$:} Deleting one edge from $R$ yields $2$ subpaths, while deleting two edges from $Z$ splits it into $2$ subpaths.
    \end{itemize}
    Let $\mathcal{H}$ be the auxiliary component graph whose vertex set is $\mathcal{P}$, where an edge $(P_i, P_j)$ exists if an edge from $\{(a,u), (b,d), (c,v)\}$ connects an endpoint in $P_i$ to an endpoint in a different subpath $P_j$.
    For each of the cut edges $(a,b),(c,d),(u,v)$, the two endpoints of the cut edge end up in different subpaths in $\mathcal{P}$.
    Therefore, each of the three added edges $(a,u),(b,d),(c,v)$ connects two different subpaths in $\mathcal{P}$.
    Furthermore, no two edges are the same since they each contain two endpoints out of the four which were originally on the same deleted edge but must now be in different subpaths.
    Since $G_{new} \in \GraphSet$ there must exist a path from $a'$ to $a''$ in $G_{new}$ and thus there must exist a path in $\mathcal{H}$ from the subpath containing $a'$ to the subpath containing $a''$.
    Thus $\mathcal{H}$ is a simple graph with $4$ vertices and $3$ edges, which is necessarily connected due to the previous facts and that every vertex in $G_{new}$ has degree two except for $a'$ and $a''$ which have degree one.
    It follows that in $G_{new}$, each of $R,Y,Z$ have been merged into a single path from $a'$ to $a''$ thus strictly reducing the number of disjoint cycles in the graph by at least one without affecting any other components in the graph.
    It thus follows that there exists a graph $H^* \in \GraphSet$ which is a Hamiltonian path over $P$.

    Since $H$ was a optimal Hamiltonian path over $P$ it follows that $|H| \leq |H^*|$ and thus $\sum_{i=1}^m (|e_i| + |e'_i|) \leq \sum_{i=1}^m (|e_i^*| + |e_i^{kept}|)$ which can be rearranged to $\sum_{i=1}^m |e_i^{deleted}| \leq \sum_{i=1}^m |e_i^*|$.
    Since every set is pairwise $s$-well-separated with $s \geq 1$ it follows from \Cref{thm:fundamental_thm_wsp} that $\forall i \in \{1, \dots, m\}, |e_i^*| \leq \frac{1}{s} |e_i|, \frac{1}{s} |e'_i|$.
    Therefore $\sum_{i=1}^m |e_i^{deleted}| \leq \sum_{i=1}^m |e_i^*| \leq \frac{1}{s} \sum_{i=1}^m |e_i^{deleted}|$ which implies that $s \leq 1$.
    If $s > 1$ then this is a contradiction and thus no optimal Hamiltonian path over $P$ can have a set transition multidigraph which contains two directed simple cycles which are symmetric with respect to each other.
    If $s = 1$ then this implies that the newest Hamiltonian path $H^*$ is also optimal.
    It then follows by construction that since $H^*$ contains strictly fewer symmetric simple cycles than $H$ that we can repeat this process until we have an optimal Hamiltonian path over $P$ which contains no symmetric simple cycles in its set transition multidigraph.
\end{proof}

\begin{thm}
    For all $k>= 2$ and $s \geq 1$ there exists two sets of $k$ points $I$ and $O$ such that all points form a metric and all points in $I$ are at most 1 unit away from each other and all points in $O$ are at least $s$ units away from any point in $I$ and there exists an optimal Hamiltonian path over $I \cup O$ which starts at a point in $I$ and ends at a point in $O$ which alternates between points in $I$ and $O$.
\end{thm}
\begin{proof}
    We can construct a metric space with $k$ points in $I$ and $k$ points in $O$ as follows:
    Let $\dist(i_a, i_b) = 1$ for all $i_a, i_b \in I$ where $i_a \neq i_b$.
    Let $\dist(i_a, o_b) = s$ for all $i_a \in I$ and $o_b \in O$.
    Let $\dist(o_a, o_b) = 2s$ for all $o_a, o_b \in O$ where $o_a \neq o_b$.
    This space clearly satisifies positivity and symmetry.
    To show that it satisfies the triangle inequality we can consider all possible combinations of three points:
    \begin{alignat*}{4}
        \bullet\quad & i_a, i_b, i_c \in I:       &\quad \dist(i_a, i_b) + \dist(i_b, i_c) &{}= 1  + 1  &&\geq 1  &&{}= \dist(i_a, i_c) \\
        \bullet\quad & i_a, i_b \in I, o_c \in O: &\quad \dist(i_a, i_b) + \dist(i_b, o_c) &{}= 1  + s  &&\geq s  &&{}= \dist(i_a, o_c) \\
        \bullet\quad & i_a \in I, o_b, o_c \in O: &\quad \dist(i_a, o_b) + \dist(o_b, o_c) &{}= s  + 2s &&\geq s  &&{}= \dist(i_a, o_c) \\
        \bullet\quad & o_a, o_b, o_c \in O:       &\quad \dist(o_a, o_b) + \dist(o_b, o_c) &{}= 2s + 2s &&\geq 2s &&{}= \dist(o_a, o_c)
    \end{alignat*}
    Thus the space is a metric.

    Consider some Hamiltonian path $P$ over $I \cup O$ which starts at a point in $I$ and ends at a point in $O$.
    Let $x$ be the number of edges in the path which connect two points in $I$.
    Let $y$ be the number of edges in the path which connect two points in $O$.
    Let $z$ be the number of edges in the path which connect a point in $I$ to a point in $O$ or vice versa.
    First note that the sum of the degrees of all $I$ nodes must be $2(k-1) + 1 = 2k - 1$ and the same will follow for $O$ nodes.
    Then it follows that $2x + z = 2k - 1$ and $2y + z = 2k - 1$ since each intra-set edge contributes $2$ to the sum of degrees and each inter-set edge contributes $1$ to the sum of degrees.
    Then it follows that $2x + z = 2y + z$ and thus $x = y$.
    Also note that $z = 2k - 1 - 2x$.

    Thus the length of the path $P$ can be expressed as:
    \begin{align*}
        |P| &= x(1) + y(2s) + z(s) \\
        &= x(1) + x(2s) + z(s)\text{ since $x = y$} \\
        &= x(1) + x(2s) + (2k - 1 - 2x)(s)\text{ since $z = 2k - 1 - 2x$} \\
        &= x + 2xs + (2k - 1)s - 2xs \\
        &= x + (2k - 1)s.
    \end{align*}
    The length of the path is minimized when $x = 0$ and thus $y = 0$ and $z = 2k - 1$.
    This implies that the optimal Hamiltonian path over $I \cup O$ must alternate between points in $I$ and $O$.
\end{proof}

\begin{corollary}
    Let $s \geq 1$, $n \geq 3$, and $k \leq n$.
    There exists a point set $P = \bigcup_{i=1}^n A_i$ in a metric space, where the subsets $A_1, \ldots, A_n$ are pairwise $s$-well-separated, such that every optimal Hamiltonian path on $P$ from $a' \in A_i$ to $a'' \in A_j$ induces a set transition multidigraph containing a node $A_l$ with in-degree and out-degree at least $k$.
\end{corollary}
% In euclidean space this may not be the case

% ZeRO fails theorem
\begin{thm}
    For all $s \geq 1$ there exists a point set $P = \bigcup_{A_i \in \ACollection} A_i$ with $|\ACollection| \geq 5$ in a metric space, where the subsets $A_1, \ldots, A_n$ are pairwise $s$-well-separated, such that every optimal Hamiltonian path on $P$ from some $a' \in A_i$ to some $a'' \in A_j$ induces a set transition multidigraph containing two disjoint paths from $A_i$ to $A_j$.
\end{thm}
\begin{proof}
    Consider the following distance matrix for some $s \geq 1$:
    $$\begin{bmatrix}
        0 & & & & & & \\
        1 & 0 & & & & & \\
        s & s & 0 & & & & \\
    ...
    \end{bmatrix}$$
    Let $A_1 = \{0, 1\}$, $A_2 = \{2, 3\}$, $A_3 = \{4\}$, $A_4 = \{5\}$, and $A_5 = \{6\}$.
    Notice that the distance matrix is symmetric and satisfies the triangle inequality and thus forms a metric space.
    The sets $A_1, A_2, A_3, A_4, A_5$ are pairwise $s$-well-separated since the distance between any two sets is at least $s$ and the diameter of each set is at most $1$.
    From node $0$ to node $3$ the unique optimal hamiltonian path is
    $$0 \xrightarrow{\; s \;} 4 \xrightarrow{\; s \;} 2 \xrightarrow{\; s \;} 5 \xrightarrow{\; s \;} 1 \xrightarrow{\; s \;} 6 \xrightarrow{\; s \;} 3.$$
    Thus the total cost of the optimal hamiltonian path is $6s$ and the induced set transition multidigraph contains two disjoint paths from $A_1$ to $A_2$: $A_1 \to A_3 \to A_2$ and $A_1 \to A_5 \to A_2$.
\end{proof}
% May not be true in euclidean space
...
\input{../MSThesis/algs/balanced_metric_split.tex}
...
\begin{lem}[Was deemed to simple]
    Let $P = \bigcup_i A_i$ be a set of points where the elements of $\ACollection = \{A_1, A_2, \ldots, A_n\}$ are all pairwise $s$-well-separated with $s \geq 1$ (and $n \geq 2$).
    Consider some hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$.
    Then every node in the set transition multidigraph $G$ induced by this Hamiltonian path is balanced except if $i \neq j$ in which case $A_i$ has one more outgoing edge than incoming edge and $A_j$ has one more incoming edge than outgoing edge.
\end{lem}
\begin{proof}
    \emph{Trivial.}
    Follows immediately from being Eulerian if $A_i = A_j$ or semi-Eulerian if $A_i \neq A_j$.
\end{proof}
...
%    %Since there are no other edges exiting $C_p$ we know that the optimal TSP tour must contain a Hamiltonian path on $C_p$ with endpoints $a_*$ and $b_*$.
%    %By Theorem~\ref{thm:wsp_path_ab} we know that this path must visit all points in $A_i$ before any point in $B_i$ (or vice versa).
%    %Thus there must be exactly one edge connecting $A_i$ to $B_i$. And exactly two edges $(a_*, d_*)$, $(b_*, e_*)$ exiting $C_p$ as desired.
