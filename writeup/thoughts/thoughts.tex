...
\begin{thm}
    ...
    Suppose that for each predefined pair of endpoints, the endpoint are always in the same set (either $A$ or $B$).
...
\end{thm}
\begin{proof} % REVIEW: can this be tightened to s >= 1 (i dont think so the general method requires breaking the cycle apart)
    To prove this theorem we will first show that the optimal tour will place all the $A$ endpoints together in the cycle order and all the $B$ endpoints together in the cycle order
    Then we will leverage the previous theorems to show that a optimal solution exists which only has two edges crossing between $A$ and $B$.

    To begin, consider some optimal solution to the problem.

    \textbf{Case 1:} All predefined endpoints are in the same set.
    Then we can fix all currently connected endpoints an apply \Cref{thm:wsp_mpath_fixed_all_a}.
    If $s > \frac{3}{2}$ then if the current solution has more than two edges crossing between $A$ and $B$ then this would be a contradiction since \Cref{thm:wsp_mpath_fixed_all_a} states that all optimal solutions must have only two edges crossing between $A$ and $B$.
    If $s = \frac{3}{2}$ then if the current solution has more than two edges crossing between $A$ and $B$ then it follows from \Cref{thm:wsp_mpath_fixed_all_a} that there exists an optimal solution with only two edges crossing between $A$ and $B$.

    \textbf{Case 2:} There are predefined endpoints in both sets.
    ...
    Suppose that two neighboring (in the cycle) non-predefined endpoints $e_j$ and $e_{j+1}$ (where $e_{2k+1} = e_1$) are such that one is in $A$ and the other is in $B$.
    Then we can note that across all neighbors $e_i,e_{i+1}$ there must be an even number of such non-predefined neighbors since in all the predefined pairs the endpoints are in the same set (and a cycle must return to the same set in the end).
    Since there is at least one predefined pair of endpoints in both sets there must be at least two such non-predefined neighboring pairs of endpoints.

    Assume by contradiction that there are more than two such non-predefined neighboring pairs of endpoints.
    Then since there is an even number of such pairs we will have at least 4 such pairs.
    We will label each pair of neigbors as follows $e'_1,e''_1; e'_2,e''_2; e'_3,e''_3; e'_4,e''_4$ where $e'_i$ and $e''_i$ are in different sets.
    WLOG assume that $e'_1, e''_2, e'_3, e''_4 \in A$ and $e''_1, e'_2, e''_3, e'_4 \in B$ (logic is symmetric).
    To better illustrate the proof we rename these endpoints as follows $a_1 = e'_1, b_1 = e''_1, a_2 = e''_2, b_2 = e'_2, a_3 = e'_3, b_3 = e''_3, a_4 = e''_4, b_4 = e'_4$.
    Finally note that on the path that goes from $a_1$ to $b_1$ there must be a edge directly connecting $A$ to $B$ and we will label this edge as $\{a^*, b^*\}$ and from $a_3$ to $b_3$ we will label its edge directly connecting $A$ to $B$ as $\{a^\dagger, b^\dagger\}$.
    Then we can note the cycle goes:
    $$\dots \leadsto \boxed{a_1 \leadsto a^* \to b^* \leadsto b_1} \leadsto \boxed{b_2 \leadsto a_2} \leadsto \boxed{a_3 \leadsto  a^\dagger \to b^\dagger \leadsto b_3} \leadsto \boxed{b_4 \leadsto a_4} \leadsto \dots.$$
    ...
    $$\dots \leadsto \boxed{a_1 \leadsto a^* \to a^\dagger \leadsto a_3} \leadsto \boxed{a_2 \leadsto b_2} \leadsto \boxed{b_1 \leadsto b^* \to b^\dagger \leadsto b_3} \leadsto \boxed{b_4 \leadsto a_4} \leadsto \dots.$$
    It can be noted that in creating this new solution no predefined pairs of endpoints are uncoupled, therefore the new solution is a valid one.
    ...
    Therefore there must be exactly two such non-predefined neighboring pairs of endpoints where one is in $A$ and the other is in $B$.

    We can then fix the endpoints of each path in the current solution and apply \Cref{thm:wsp_mpath_fixed_mixed}.
    Note that only two paths will have their endpoints fixed to be in different sets based on the previous analysis.
    If $s > \frac{3}{2}$ then the current solution must have only two edges crossing between $A$ and $B$ since if it had more than two edges crossing between $A$ and $B$ then this would be a contradiction to \Cref{thm:wsp_mpath_fixed_mixed} which states that all optimal solutions must have only two edges crossing between $A$ and $B$.
    If $s = \frac{3}{2}$ then by \Cref{thm:wsp_mpath_fixed_mixed} there must exist an optimal solution which uses the same fixing of endpoints but only has two edges crossing between $A$ and $B$.
