...
\begin{proof}
    ...
    Notice that for any of the $2^m$ possible permutations of deletions, the degree of every vertex in $P$ remains exactly as it was in the original Hamiltonian path $H$.
    Specifically, the endpoints $a'$ and $a''$ retain a degree of 1, and all other internal vertices retain a degree of 2.
    Because these degrees are perfectly preserved, any resulting graph from our deletion and replacement scheme must consist of exactly one path (from $a'$ to $a''$) and $c \geq 0$ disjoint cycles.
    ...
    There must exist at least one permutation of deletions such that the resulting graph contains no cycles.
    ...
    After the additions and deletions the affected components $\{R, Z, Y\}$ are partitioned into exactly four disjoint connected subpaths, which we denote $\PathSet = \{P_1, P_2, P_3, P_4\}$. 
    ...
    Thus $\mathcal{H}$ is a simple graph with $4$ vertices and $3$ edges, which is necessarily connected.
...
%\begin{proof}
%    Consider the induced Eulerian walk of some optimal Hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$.
%    Suppose the two parallel paths start on a vertex $A \in \ACollection$ and end on a vertex $C \in \ACollection$ where one path is of length one and the other is of length two.
%    Let the intermediate vertex on the length two path be $B \in \ACollection$.
%    WLOG assume the length one path is traversed first in the Eulerian walk.
%    We will label the following edges in the Hamiltonian path over $P$ as follows $(a_1, c_1), (a_2, b_1), (b_2, c_2)$ where $a_i \in A$, $b_i \in B$, and $c_i \in C$ each corresponding to the edges in the induced Eulerian walk.
%\end{proof}
