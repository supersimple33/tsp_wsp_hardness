...
\begin{defn}
    ...
    We define the \textbf{Set Transition Multidigraph} $G = (V,E)$ induced by some Hamiltonian path over $P$ to be the simple multidigraph where $V = \{A_1, A_2, \ldots, A_n\}$ and there is an edge from $A_i$ to $A_j$ for every time the Hamiltonian path over $P$ transitions from a point in $A_i$ and to a point in $A_j$ for $i \neq j$.
\end{defn}

\begin{lem}\label{lem:set_transition_multidigraph_degree}
    Let $P = \bigcup_i A_i$ be a set of points where the elements of $\ACollection = \{A_1, A_2, \ldots, A_n\}$ are all pairwise $s$-well-separated with $s \geq 1$ (and $n \geq 2$).
...
\begin{proof}
    ...
    If the digraph has six edges then all three nodes are balanced and this would violate \Cref{lem:set_transition_multidigraph_degree}.
...
\begin{thm}
    Let $P = \bigcup_i A_i$ be a set of points where the elements of $\ACollection = \{A_1, A_2, \ldots, A_n\}$ are all pairwise $s$-well-separated with $s \geq 1$ (and $n \geq 2$).
    Then there exists an optimal Hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$ such that the set transition multidigraph  induced by the 
\end{thm}
