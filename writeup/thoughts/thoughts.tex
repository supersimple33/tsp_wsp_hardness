...
\begin{proof}
    ...
    Assume by contradiction that there is a path $P_i$ in the solution that has an edge connecting $A$ to $B$.
    ...
    Because there is a even number of predefined pairs with one point in $A$ and the other in $B$ then the number of paths with one endpoint in $A$ and the other in $B$ must also be even to ensure that the total number of alternations between $A$ and $B$ along the cycle is even.
    ...
    By linking $a^*$ to $a^\dagger$ and $b^*$ to $b^\dagger$ we can break apart the solution at the two edges $\{a^*, b^*\}$ and $\{b^\dagger, a^\dagger\}$ and create two disjoint sub-cycles, $C_1$ and $C_2$ of which one begins and ends in $A$ and the other begins and ends in $B$.
    WLOG assume that $C_1$ contains at least one predefined pair of endpoints where one endpoint is in $A$ and the other is in $B$ (if not then $C_2$ must contain at least one such predefined pair).
    Since all alternations must be done via predefined pairs of endpoints it follows that $C_1$ must contain another predefined pair of endpoints whose orientation in the original solution is opposite the first pair (otherwise a contradiction would arise since $C_1$ would not be able to return to the same set it started in or would have a path other than $P_i$ and $P_j$ crossing between $A$ and $B$).

    WLOG assume that $C_1$ contains $a'$ and $a''$ and $C_2$ contains $b'$ and $b''$ (the logic is symmetric if the opposite is true).
    Let $(a_1, b_1)$ be the first predefined pair of endpoints in $C_1$ with $a_1 \in A$ and $b_1 \in B$.
    Let $b_2 \in B$ be the endpoint on the other end of the path that begins with $b_1$.
    Next consider this path and any other path in the solution that has at least one endpoint in $A$.
    It follows from \Cref{lem:wsp_twopath_fixed_aa_ab,lem:wsp_twopath_fixed_aa_bb} that there exists an optimal solution to the problem where the path with $b_1,b_2$ has no points in $A$. 
    Therefore we can choose any edge from the $b_1$ to $b_2$ path and it will always conenct two points in $B$.
    Considering the first edge in the $b_1$ to $b_2$, it follows that the first edge must connect $b_1$ to some point in $B$ and we will label this point $b_3$.
    ...
    Since the previous solution was assumed optimal we know that $|a^* b^*| + |b^\dagger a^\dagger| + |b_1 b_3| \leq |a^* a^\dagger| + |b^* b^\dagger| + |b_1 b_3|$.
    ...
    $$
        ...
        |b^* b^\dagger| \leq \frac{1}{s} \omega \quad
        |b_1 b_3| \leq \frac{1}{s} \omega.
    $$
    From these we can conclude: $2\omega \leq |a^* b^*| + |b^\dagger a^\dagger| + |b_1 b_3| \leq |a^* a^\dagger| + |b^* b^\dagger| + |b_1 b_3| \leq \frac{3}{s} \omega$ which implies that $\frac{3}{2} \leq s$.
    If $s > \frac{3}{2}$ then this is a contradiction and thus there cannot be any paths with edges connecting $A$ to $B$.
    If $s = \frac{3}{2}$ then we can note that all these inequalities must be tight and thus there exists an optimal solution where no path has an edge connecting $A$ to $B$ as desired.

    Since the solution with no paths having endpoints in both $A$ and $B$ is optimal it is also an optimal solution to the multiple optimal Hamiltonian paths problem where the endpoints of each path are fixed in the same way as in the current solution.
    If $s > \frac{3}{2}$ then the current solution must have no edges crossing between $A$ and $B$ by \Cref{thm:wsp_mpath_fixed_mixed}.
    If $s = \frac{3}{2}$ it follows that for the given configuration of endpoints there exists an optimal solution where no path has an edge connecting $A$ to $B$ by \Cref{thm:wsp_mpath_fixed_mixed}.
\end{proof}
