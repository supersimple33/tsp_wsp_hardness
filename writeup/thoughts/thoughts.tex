...
\begin{lem}\label{lem:wsp_twopath_fixed_aa_aa}
...
\begin{lem}\label{lem:wsp_twopath_fixed_aa_ab}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$.
...
\begin{lem}\label{lem:wsp_twopath_fixed_ab_ab}
...
\begin{lem}\label{lem:wsp_twopath_fixed_aa_bb}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$.
...
\begin{thm}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$. 
    Then the multiple optimal hamiltonian paths problem of $k \geq 2$ paths where each paths endpoints are fixed and at least one path has an endpoint in $A$ and at least one path has an endpoing in $B$ has a solution where paths which have both an endpoint in $A$ and an endpoint in $B$ have a single edge connecting $A$ to $B$ and all other paths only have points in $A$ or only have points in $B$ (if they start/end in $A$ resp $B$).
    Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
    ...
    WLOG assume define that endpoints of the path to be $a',a'' \in A$ (otherwise just flip the names for $A$ and $B$).
