...
\subsubsection{Single WSP Entry/Exit Fixed Multi Path Analysis}
...
\begin{corollary}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. Then the multiple optimal tsp problem of $k \geq 2$ tours which has all starting and ending nodes in $A$ has a optimal solution where all tours but one stay within $A$ and the remaining tour visits all points in $B$ consecutively. Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{corollary}
...
\begin{corollary}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$. Then the multiple optimal travelling salesman problem of $k \geq 2$ paths which has a mixture of starting nodes in $A$ and $B$ has a solution where there are no edges connecting $A$ to $B$ in any of the paths. Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{corollary}
