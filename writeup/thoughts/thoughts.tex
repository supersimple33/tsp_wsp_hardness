...
\begin{corollary}
    If $A$ and $B$ are $s$-well-separated then $\dist(A,B) > 0$.
\end{corollary}
...
\begin{thm}\label{thm:wsp_path_aa}
    ...
    Then there exists an optimal hamiltonian path between endpoints $a', a'' \in A$ over $A \cup B$ that visits all points in $B$ consecutively.
    Furthermore if $s > 1$ then all optimal hamiltonian paths must visit all points in $B$ consecutively.
\end{thm}
\begin{proof}
    ...
    Since $A$ and $B$ are $s$-well-separated we know that $\forall i,j,k; |a_i a_j| \leq \frac{1}{s} |a_i b_k|$ (and vice versa for $B$). From this we can conclude the following:
    \begin{align*}
        |b_2 a_2| + |b_4 a_4| &\leq |b_2 b_4| + |a_2 a_4| \\
    ...
    Since the new path has strictly fewer alternating loops between $A$ and $B$ than the original path, it follows from induction that all but the required two edges crossing between $A$ and $B$ can be removed to construct an optimal hamiltonian path that visits all points in $B$ consecutively.
\end{proof}

\begin{corollary}\label{cor:wsp_tsp}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$.
    Then there exists an optimal TSP tour on $A \cup B$ that visits all points in $A$ before any point in $B$ (or vice versa).
    Furthermore if $s > 1$ then all optimal TSP tours must satisfy this property.
\end{corollary}
...
\begin{proof} % gemini checked
    ...
    $(a^\dagger_1, b^\dagger_1), (b^\dagger_2, a^\dagger_2)$ all ordered as they appear in their respective paths.
    Let the first path have endpoints $a'_s$ and $a'_e$ and let the second path have endpoints $a''_s$ and $a''_e$.
    ...
    \quad\quad
    a''_s \leadsto a^\dagger_1 \to b^\dagger_1 \leadsto b^\dagger_2 \to b^*_1 \leadsto b^*_2 \to a^*_2 \leadsto a'_e$$
    ...
    Since the new path has strictly fewer alternating loops between $A$ and $B$, it follows via induction that there must exist an optimal solution where one path only has points in $A$ and another visits all points in $B$ consecutively.
...
\begin{lem}\label{lem:wsp_twopath_aa_ab}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. 
    Then the multiple optimal hamiltonian paths problem solution between starting nodes $a'_s,a''_s \in A$ and ending nodes $a'_e \in A$ and $b'_e \in B$ has a solution where one path only has points in $A$ and another has exactly one edge connecting $A$ to $B$. 
    Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
    Assume that for some solution to the problem the two optimal paths each visit some points in $B$.
    ...
    Since the new path has strictly fewer alternating loops between $A$ and $B$ then by induction we can construct an optimal solution where one path only has points in $A$ and another visits all points in $B$.
...
\begin{lem}\label{lem:wsp_twopath_aa_bb}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$.
    Then the multiple optimal hamiltonian paths problem solution between starting nodes $a'_s,a''_s \in A$ and ending nodes $b'_e,b''_e \in B$ has a solution where each path has exactly one edge connecting $A$ to $B$.
    Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
...
\begin{lem}\label{lem:wsp_twopath_ab_ab}
    ...
    Then the optimal multiple hamiltonian paths problem solution between starting nodes $a'_s \in A$ and $b'_s \in B$ and ending nodes $a'_e \in A$ and $b'_e \in B$ has a solution where each neither path has a edge connecting $A$ to $B$. 
...
\begin{thm}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. 
    Then the multiple optimal hamiltonian paths problem of $k \geq 2$ paths which has all starting and ending nodes in $A$ has a optimal solution where all paths but one stay within $A$ and the remaining path visits all points in $B$ consecutively. 
    Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
...
\begin{thm}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$. 
    Then the multiple optimal hamiltonian paths problem of $k \geq 2$ paths which has $n$ starting nodes in $A$ and $m$ ending nodes in $A$ with $\min(n,m) \neq k$ and $0 \neq \max(n,m)$ has a solution where there are $|n-m|$ paths with exactly one edge connecting $A$ to $B$. 
    There are $\min(n,m)$ paths with only points in $A$ and there are $k - \max(n,m)$ paths with only points in $B$.
    Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
...
\begin{lem}\label{lem:wsp_twopath_aaab}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$.
    Then the multiple optimal hamiltonian paths problem solution between endpoints $a',a'',a''' \in A$ and $b' \in B$ has a solution where one path only has points in $A$ and another has exactly one edge connecting $A$ to $B$.
    Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
...
\begin{lem}\label{lem:wsp_twopath_aabb}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$.
    Then the multiple optimal hamiltonian paths problem solution over endpoints $a',a'' \in A$ and $b',b'' \in B$ has a solution where there are no edges connecting $A$ to $B$.
    Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
...
\begin{lem}\label{lem:wsp_twopath_aaaa}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$.
    Then the multiple optimal hamiltonian paths problem solution between endpoints $a',a'',a''',a'''' \in A$ has a solution where only a single path has two edges connecting $A$ to $B$ and the other path has no edges connecting $A$ to $B$.
    Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
