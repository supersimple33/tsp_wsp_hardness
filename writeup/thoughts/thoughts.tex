...
\begin{thm}
    Suppose sets $A$ and $B$ are $s$-well-separated for $s \geq \frac{3}{2}$.
    Consider a multiple optimal Hamiltonian paths problem of $k \geq 2$ paths over $2k$ endpoints, organized into $k$ pairs of endpoints; where the routing must be chosen such that alternating between the paths and the paired endpoints forms exactly one continuous cycle.
    Suppose that there is at least one endpoint in $A$ and at least one endpoint in $B$.
    Furthermore suppose that there is an odd number of pairs where one endpoint is in $A$ and the other is in $B$.
