...
\begin{thm}\label{thm:wsp_path_ab}
    ...
    Then there exists an optimal hamiltonian path between endpoints $a' \in A$ and $b' \in B$ over $A \cup B$ that visits all points in $A$ before any point in $B$.
    Furthermore if $s > 1$ then all optimal hamiltonians must visit all points in $A$ before any point in $B$.
\end{thm}
\begin{proof}
