...
\begin{proof}
    ...
    %See Veblen's Theorem \autocite{veblenApplicationModularEquations1912}.
\end{proof}
...
\begin{lem}\label{lem:direct_k_wsp_path}
    Let $P = \bigcup_{i=1}^n A_i$ be a set of points where $A_1, A_2, \ldots, A_n$ are all pairwise $s$-well-separated with $s \geq 1.5$ (and $n \geq 3$).
    Then there exists an optimal Hamiltonian path over $P$ which starts at $a' \in A_i$ and ends at $a'' \in A_j$ where its induced Eulerian walk over the set 
\end{lem}
\begin{proof}
