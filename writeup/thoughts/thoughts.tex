...
\begin{thm}
    Given a set of points $P$ within a ball of diameter $d$ within a metric space and starting point $x,y \in P$. Let $L$ be the length of some optimal hamiltonian path $H$ between $x$ and $y$ over $P$. Then the length of the optimal hamiltonian path after removing $k$ intermediate points is at least $L - d(k+b)$ where $b$ is the number of non-contiguous blocks of points removed (under $H$).
\end{thm}
\begin{proof}
...
\begin{corollary}
     Given a set of points $P$ within a ball of diameter $d$ within a metric space and starting point $x,y \in P$. Let $L$ be the length of some optimal hamiltonian path $H$ between $x$ and $y$ over $P$. Then the length of the optimal hamiltonian path after removing $k$ intermediate points is at least $L - 2dk$.
\end{corollary}
...
\begin{lem}\label{lem:wsp_twopath_aa_aa}
...
\begin{lem}\label{lem:wsp_twopath_ab_ab}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$. Then the optimal multiple hamiltonian paths problem solution between starting nodes $a'_s \in A$ and $b'_s \in B$ and ending nodes $a'_e \in A$ and $b'_e \in B$ has a solution where each neither path has a edge connecting $A$ to $B$. Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property. 
    Furthermore no optimal solution can have both paths connecting $A$ and $B$.
\end{lem}
\begin{proof}
    Assume that for some solution one of its path has an edge connecting $A$ to $B$.

    \textbf{Case 1:} The path has one endpoint in $A$ and the other in $B$.
    Let the first path have endpoints $a'_s$ and $b'_e$ and let the second path have endpoints $b'_s$ and $a'_e$.
    Then we can note that there must be the following edges in the path:
    $(a^*_1, b^*_1)$ in the first path and in the other path $(b^\dagger_1, a^\dagger_1)$ all ordered as they appear in their respective paths.
    This gives the following partial structure of the paths:
    $$a'_s \leadsto a^*_1 \to b^*_1 \leadsto b'_e
    \quad\quad
    b'_s \leadsto b^\dagger_1 \to a^\dagger_1 \leadsto a'_e$$

    Since the paths are optimal we know that they satisfy the 2-optimality condition implying the following:
    $|a^*_1 b^*_1| + |b^\dagger_1 a^\dagger_1| \leq |a^*_1 a^\dagger_1| + |b^\dagger_1 b^*_1|$.
    This is to say that the following paths must have a combined length at least as long as the original paths:
    $$a'_s \leadsto a^*_1 \to a^\dagger_1 \leadsto a'_e
    \quad\quad
    b'_s \leadsto b^\dagger_1 \to b^*_1 \leadsto b'_e$$

    Since $A$ and $B$ are $s$-well-separated we know that $\forall i,j; |a_i a_j| \leq \frac{1}{s} |a_i b_i|$. From this we can conclude the following:
    \begin{align*}
        |a^*_1 b^*_1| + |b^\dagger_1 a^\dagger_1| &\leq |a^*_1 a^\dagger_1| + |b^\dagger_1 b^*_1| \\
        &\leq \frac{1}{s} |a^*_1 b^*_1| + \frac{1}{s} |b^\dagger_1 a^\dagger_1| \\
        &= \frac{1}{s} (|a^*_1 b^*_1| + |b^\dagger_1 a^\dagger_1|)
    \end{align*}
    This implies that $1 \leq \frac{1}{s}$ which in turn implies that $s \leq 1$.
    This however is a contradiction since we assumed that $s \geq \frac{3}{2}$.

    \textbf{Case 2:} The path has both endpoints in either $A$ xor $B$.
    WLOG assume that the path which starts at $a'_s$ and ends at $a'_e$ and traverses part of $B$.
    We will call this path $P_A$ and the other path $P_B$.
    
    \textbf{Case 2a:} $P_B$ also traverses part of $A$.
    ...
    This is to say that the following paths must have a combined length at least as long as the original paths:
    $$a'_s \leadsto a^*_1 \to a^\dagger_1 \leadsto a^\dagger_2 \leadsto a'_e
    \quad\quad
    ...
    \textbf{Case 2b:} $P_B$ does not traverse any part of $A$.
    ...
    We can define $L$ to be the total length of the two paths as is. We can then build a new solution by shortcutting the path by directly connecting $a^*_1$ to $a^*_2$ within $P_A$ to form $P'_A$ and we can break the edge $(b^\dagger_1, b^\dagger_2)$ in $P_B$ and then connect $b^\dagger_1$ to $b^*_1$ and $b^*_2$ to $b^\dagger_2$ to form $P'_B$.
    ...
    Using the separation bounds we have:
    \[
        |a^*_1b^*_1| \ge \delta,\quad |b^*_2a^*_2| \ge \delta,\quad
        |a^*_1a^*_2|\le \frac{\delta}{s},\quad |b^\dagger_1 b^*_1|\le \frac{\delta}{s},\quad
        |b^*_2 b^\dagger_2|\le \frac{\delta}{s},\quad |b^\dagger_1 b^\dagger_2|\ge 0,
    \]
    ...
    \begin{align*}
        ...
        &\geq 2\delta - (|a^*_1 a^*_2| + |b^\dagger_1 b^*_1| + |b^*_2 b^\dagger_2|) \\
        &\geq 2\delta - \frac{3\delta}{s} \\
        &= \delta \left(2 - \frac{3}{s}\right)
    \end{align*}
    ...
    If $s = \frac{3}{2}$ then we have that $L - L' \geq 0$ which combined with the optimality of the original paths implies that $L - L' = 0$ which implies that the newly constructed paths are also optimal.
    Therefore starting from any solution where one of the paths has an edge connecting $A$ to $B$ we can repeatedly apply this logic to construct a solution where neither path has an edge connecting $A$ to $B$.

    Combining case 1 and case 2 we have that if $s > \frac{3}{2}$ then all optimal solutions must satisfy the property that neither path has an edge connecting $A$ to $B$.
    Instead if $s = \frac{3}{2}$ then there exists an optimal solution where neither path has an edge connecting $A$ to $B$.
    It further follows from case 1 and case 2a that no optimal solution can have both paths connecting $A$ and $B$.
\end{proof}

\begin{rem*}
    The bound on $s$ in the previous lemma is tight.
\end{rem*}
\begin{proof}
    Let $s < \frac{3}{2}$.
    Let $a_1 = \left(0,0\right), a_2 = \left(0,\frac{2}{3}\right)$ and $b_1 = \left(\frac{2\sqrt{2}}{3}+\frac{1}{s},\frac{1}{3}\right), b_2 = \left(\frac{2\sqrt{2}}{3},\frac{1}{3}\right), b_3 = \left(\frac{2\sqrt{2}}{3}+\frac{1}{s},\frac{1}{3}\right)$.
    Next we define $A = \{a_1, a_2\}$ and $B = \{b_1, b_2, b_3\}$.
    We can then note that the diameter of $A$ is $\frac{2}{3}$ and the diameter of $B$ is $\frac{1}{s}$.
    The distance between the two sets is $1$ since $|a_1 b_2| = |a_2 b_2| = 1$ and all other distances between points in $A$ and $B$ are strictly greater than $1$.
    Thus the separation factor of these two sets is $\frac{1}{\frac{1}{s}} = s$.
    
    Consider the multiple optimal hamiltonian paths problem between starting nodes $a_1, b_1$ and ending nodes $a_2, b_3$.
    We can then compare the following two solutions to this problem (which represent the only two viable solutions):
    \begin{itemize}
        \item $P_A: a_1 \to a_2$ and $P_B: b_1 \to b_2 \to b_3$
        \item $P'_A: a_1 \to b_2 \to a_2$ and $P'_B: b_1 \to b_3$
    \end{itemize}

    We can then note that the total length of the second solution is $1+1 = 2$.
    The total length of the first solution is $\frac{2}{3} + \frac{1}{s} + \frac{1}{s} = \frac{2}{3} + \frac{2}{s} > 2$ since $s < \frac{3}{2}$.
    Therefore the first solution is not optimal and thus there does not exist an optimal solution where neither path has an edge connecting $A$ to $B$.
\end{proof}

\begin{lem}\label{lem:wsp_twopath_aa_ab}
...
\begin{lem}\label{lem:wsp_twopath_aa_bb}
...
\begin{thm}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. Then the multiple optimal hamiltonian paths problem of $k \geq 2$ paths which has all starting and ending nodes in $A$ has a optimal solution where all paths but one stay within $A$ and the remaining path visits all points in $B$ consecutively. Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
    Suppose that for some optimal solution there are at least two paths that visit some points in $B$.
    ...
    Then we can note that $A'$ and $B'$ are still $s$-well-separated since they are subsets of $A$ and $B$ respectively.
    If $s > 1$ then this would cause a contradiction as such a solution would also be optimal on the subproblem and thus violate \Cref{lem:wsp_twopath_aa_aa}.
    If $s = 1$ then we can apply \Cref{lem:wsp_twopath_aa_aa} to construct a solution to the subproblem where one path visits only points in $A'$ and the other path visits all points in $B'$ consecutively.
    
    We can then apply this logic repeatedly to construct a solution where only one path visits some points in $B$ and all other paths only have points in $A$.
    We can then apply \Cref{thm:wsp_path_aa} to the path that visits some points in $B$ to construct a solution where this path visits all points in $B$ consecutively and furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{proof}

\begin{corollary} % follows since deviation would cause contradiction
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. Then the multiple optimal travelling salesman problem of $k \geq 2$ tours which has all starting and ending nodes in $A$ has a optimal solution where all tours but one stay within $A$ and the remaining tour visits all points in $B$ consecutively. Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{corollary}

\begin{thm}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$. Then the multiple optimal hamiltonian paths problem of $k \geq 2$ paths which has $n$ starting nodes in $A$ and $m$ ending nodes in $A$ with $\min(n,m) \neq k$ and $0 \neq \max(n,m)$ has a solution where there are $|n-m|$ paths with exactly one edge connecting $A$ to $B$. There are $\min(n,m)$ paths with only points in $A$ and there are $k - \max(n,m)$ paths with only points in $B$. Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
    Say we take a given optimal solution to the problem.

    \textbf{Case 1:} There are not $|n-m|$ paths with one endpoint in $A$ and the other in $B$.

    \textbf{Case 1a:} There are less than $|n-m|$ paths with one endpoint in $A$ and the other in $B$.
    This is impossible since if there are less than $|n-m|$ paths with one endpoint in $A$ and the other in $B$ then there must be more than $\min(n,m)$ paths with only endpoints points in $A$ or more than $k - \max(n,m)$ paths with only endpoints in $B$ which is a contradiction since those values represent the maximum number of paths that can have only endpoints in $A$ and $B$ respectively.

    \textbf{Case 1b:} There are more than $|n-m|$ paths with one endpoint in $A$ and the other in $B$.
    In order for this extra path to come about there must be at least one less path from the $\min(n,m)$ paths with only points in $A$ and one less path from the $k - \max(n,m)$ paths with only points in $B$.
    This would then imply that there are at least two extra paths with one endpoint in $A$ and the other in $B$.
    However by \Cref{lem:wsp_twopath_ab_ab} we know that if $s > \frac{3}{2}$ then there cannot be two paths with one endpoint in $A$ and the other in $B$ and if $s = \frac{3}{2}$ then there exists an optimal solution where these two extra paths are replaced by two paths that start and end in the same sets (one in $A$ and one in $B$).
    It follows from structural induction that if $s = \frac{3}{2}$ then there exists an optimal solution where there are exactly $|n-m|$ paths with one endpoint in $A$ and the other in $B$.

    \textbf{Case 2:} There are less than $\min(n,m)$ paths with only endpoints points in $A$ or less than $k - \max(n,m)$ paths with only endpoints in $B$.
    For this to be the case there must be at least one more path with one endpoint in $A$ and the other in $B$ than $|n-m|$ which is a contradiction since then case 1. would have applied.

    \textbf{Case 3:} There are exactly $|n-m|$ paths with one endpoint in $A$ and the other in $B$ and there are exactly $\min(n,m)$ paths with only endpoints points in $A$ and exactly $k - \max(n,m)$ paths with only endpoints in $B$.

    \textbf{Case 3a:} There is a path with one endpoint in $A$ and the other in $B$ that has more than one edge connecting $A$ to $B$.
    This is impossible since by \Cref{thm:wsp_path_ab} we know that such a path cannot be optimal when $s > 1$.

    \textbf{Case 3b:} There is a path with both endpoints in $A$ xor $B$ that has an edge connecting $A$ to $B$.
    WLOG assume that there is a path with both endpoints in $A$ that has an edge connecting $A$ to $B$.
    Since $\min(n,m) \neq k$ and $0 \neq \max(n,m)$ we know that there must exist at least one other path with an endpoint from $B$

    \textbf{Case 3b.i:} $|m - n| > 0$.
    We can then choose a single path from the $|n-m|$ paths with one endpoint in $A$ and the other in $B$ and add it to the path with both endpoints in $A$ that has an edge connecting $A$ to $B$ to construct a new multiple hamiltonian paths problem of the points from these two paths.
    It then follows from \Cref{lem:wsp_twopath_aa_ab} that if the optimal solution to this new problem had the original path with both endpoints in $A$ travelling to $B$ this would be a contradiction since such a path cannot be optimal when $s > 1$.

    \textbf{Case 3b.ii:} $|m - n| = 0$.
    Then since another path must exist this path must have both endpoints in $B$.
    We can then form a new multiple hamiltonian paths problem of the points from these two paths.
    It then follows from \Cref{lem:wsp_twopath_ab_ab} that if $s > \frac{3}{2}$ then having the original path cross into $B$ is impossible.
    If $s = \frac{3}{2}$ then there exists an optimal solution to this new problem where neither path has an edge connecting $A$ to $B$.
    It then follows by structural induction that for all paths with the endpoints in the same set (either $A$ or $B$) there exists an optimal solution where none of these paths have an edge connecting $A$ to $B$.

    If $s = \frac{3}{2}$ then there exists an optimal solution that satisfies the property that there are $|n-m|$ paths with exactly one edge connecting $A$ to $B$, there are $\min(n,m)$ paths with only points in $A$ and there are $k - \max(n,m)$ paths with only points in $B$.
    If $s > \frac{3}{2}$ then all optimal solutions must satisfy the previous properties.
\end{proof}

\begin{corollary} % follows since deviation would cause contradiction
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$. Then the multiple optimal travelling salesman problem of $k \geq 2$ paths which has a mixture of starting nodes in $A$ and $B$ has a solution where there are no edges connecting $A$ to $B$ in any of the tours. Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{corollary}

\subsubsection{Single WSP Endpoint Set Multi Path Analysis}

\begin{lem}\label{lem:wsp_twopath_aaab}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. Then the multiple optimal hamiltonian paths problem solution between endpoints $a',a'',a''' \in A$ and $b' \in B$ has a solution where one path only has points in $A$ and another has exactly one edge connecting $A$ to $B$. Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
    WLOG assume that for some optimal solution to the problem we have one path with endpoints $a', a''$ and another path with endpoints $a''', b'$ (if this is not the case then we can just shuffle the $a$ endpoint names).
    If $s = 1$ and the solution has both paths visiting some points in $B$ then we can apply \Cref{lem:wsp_twopath_aa_ab} to construct a optimal solution where one path only has points in $A$ and the other path visits all points in $B$ consecutively.
    If $s > 1$ and the solution doesn't have a path that visits all of $B$ consecutively then this would be a contradiction since by \Cref{lem:wsp_twopath_aa_ab} we know that generating a optimal path with starting nodes $a', a'''$ and ending nodes $a'', b'$ that has more than one edge connecting $A$ to $B$ is impossible.
\end{proof}

\begin{lem}\label{lem:wsp_twopath_aabb}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$. Then the multiple optimal hamiltonian paths problem solution over endpoints $a',a'' \in A$ and $b',b'' \in B$ has a solution where there are no edges connecting $A$ to $B$. Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
    Consider two labellings for the endpoints the first where $a', b'$ are required starting nodes and $a'', b''$ are required ending nodes and the second where $a', b''$ are required starting nodes and $a'', b'$ are required ending nodes.

    Then any solution must fall into one of the previous cases.
    It follows from \Cref{lem:wsp_twopath_ab_ab} that under either case if $s > \frac{3}{2}$ then any optimal solution must have no edges connecting $A$ to $B$ and if $s = \frac{3}{2}$ then there exists an optimal solution where there are no edges connecting $A$ to $B$.
\end{proof}

\begin{lem}\label{lem:wsp_twopath_aaaa}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. Then the multiple optimal hamiltonian paths problem solution between endpoints $a',a'',a''',a'''' \in A$ has a solution where only a single path has two edges connecting $A$ to $B$ and the other path has no edges connecting $A$ to $B$. Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{lem}
\begin{proof}
    Consider two labellings for the endpoints the first where $a', a''$ are required starting nodes and $a''', a''''$ are required ending nodes and the second where $a', a'''$ are required starting nodes and $a'', a''''$ are required ending nodes.

    Then any solution must fall into one of the previous cases.
    It follows from \Cref{lem:wsp_twopath_aa_aa} that under either case if $s > 1$ then any optimal solution must exactly two edges connecting $A$ to $B$ and if $s = 1$ then there exists an optimal solution where there are exactly two edges connecting $A$ to $B$.
\end{proof}

\begin{thm}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$ and we have a multiple optimal hamiltonian paths problem with $k \geq 2$ paths where all endpoints are in $A$. Then there exists an optimal solution where a single path visits all points in $B$ consecutively and all other paths only have points in $A$. Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
...
\begin{thm} 
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$ and we have a multiple optimal hamiltonian paths problem with $k \geq 2$ paths where there are an odd number of endpoints in $A$. Then there exists an optimal solution where a single path has exactly one edge connecting $A$ to $B$ and all other paths have no edges connecting $A$ to $B$. Furthermore if $s > 1$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
    Consider some optimal solution to the problem.
    ...
    Since $A$ and $B$ are $s$-well-separated we know that $\forall i,j; |a_i a_j| \leq \frac{1}{s} |a_i b_i|$. 
...
\begin{thm}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq \frac{3}{2}$ and we have a multiple optimal hamiltonian paths problem with $k \geq 2$ paths where there are a non-zero even number of endpoints in $A$ and a non-zero even number of endpoints in $B$. Then there exists an optimal solution where there are no edges connecting $A$ to $B$ in any of the paths. Furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
\end{thm}
\begin{proof}
    Consider some optimal solution to the problem.
    Suppose that there are at least two paths that have endpoints in different sets (one in $A$ and the other in $B$).
...
\begin{thm}\label{thm:bin_wspd_tour_structure}
    ...
    Then there must exist a optimal TSP tour over $P$ such that for any $k$-binary WSP $(A_i, B_i) \in \PairDecomp_s$ there is exactly zero, one, or two edges in the tour connecting $A_i,B_i$ and correspondingly there are either four, two, or two edges exiting $A_i \cup B_i$ respectively if they are not a $1$-binary WSP (aka the root pair).
