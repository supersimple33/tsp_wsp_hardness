...
\input{../MSThesis/theorems/wsp_mtour_all_a.tex}
...
\end{lem}
\begin{proof}
    Assume that for some optimal solution it has a edge connecting $A$ to $B$.

    \textbf{Case 1:} The path with an edge connecting $A$ to $B$ has endpoints in different sets.
    Then the other path must also have endpoints in different sets.
    WLOG let one path have endpoints $a',b'$ and the other have endpoints $a'', b''$ (if not then we can shuffle the endpoint names).
    ...
    This gives the following partial structure to the paths:
    $$a' \leadsto a^* \to b^* \leadsto b' \quad\quad a'' \leadsto a^\dagger \to b^\dagger \leadsto b''$$

    Since the paths are optimal we know that they satisfy the 2-optimality condition implying that $|a^* b^*| + |a^\dagger b^\dagger| \leq |a^* a^\dagger| + |b^* b^\dagger|$.
    That is to say the following paths must have a combined length at least as long as the original paths:
    ...
    If $s > 1$ this is a contradiction and therefore any solution where a path has endpoints in different sets cannot be optimal.
    ...
    \textbf{Case 2:} The path with an edge connecting $A$ to $B$ has endpoints in the same set.
    Then the other path must also have endpoints in the same set.
    WLOG let one path have endpoints $a', a''$ and the other have endpoints $b', b''$ (if not then we can shuffle the endpoint names).
    We can then label $a',b'$ as the starting nodes and $a'', b''$ as the ending nodes.
    It then follows from \Cref{lem:wsp_twopath_ab_ab} that we can construct a solution where there are no edges connecting $A$ to $B$ and furthermore if $s > \frac{3}{2}$ then all optimal solutions must satisfy this property.
    
    If $s > \frac{3}{2}$ then either case leads to a contradiction and thus all optimal solutions must have no edges connecting $A$ to $B$.
    If $s = \frac{3}{2}$ then either case leads to a optimal solution where there are no edges connecting $A$ to $B$.
\end{proof}
