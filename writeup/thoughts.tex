...
\begin{lem}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s \geq 1$. Then the multiple optimal hamiltonian paths problem solution between starting nodes $a'_s,a''_s \in A$ and ending nodes $a'_e, a''_e \in A$ has a solution where one path only has points in $A$ and another visits all points in $B$ consecutively. Furthermore if $s > 1$ then all optimal hamiltonian paths must satisfy this property.
\end{lem}
\begin{proof}
    ...
    $(a^*_1, b^*_1), (b^*_2, a^*_2)$ and the following edges in the second path: 
    ...
    It follows via induction that all the extra loops between $A$ and $B$ can be removed from one of the paths to construct a solution where one path only has points in $A$ and another visits all points in $B$ consecutively.

    From here we can take the path that visits all the points in $B$ and take all the points it visits in $A$ and label this as $A'$.
