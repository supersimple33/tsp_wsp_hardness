...
\begin{thm}\label{thm:wsp_path_ab}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s > 1$. Then the optimal hamiltonian path on $A \cup B$ with endpoints $a' \in A$ and $b' \in B$ must visit all points in $A$ before any point in $B$.
\end{thm}
\begin{proof}
    Let $|A| = n$ and $|B| = m$.
...
\begin{thm}\label{thm:wsp_path_aa}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s > 1$. Then the optimal hamiltonian path on $A \cup B$ with endpoints $a',a'' \in A$ must only contain two edges between $A$ and $B$.
\end{thm}
\begin{proof}
    Let $|A| = n$ and $|B| = m$.
...
\begin{corollary}\label{cor:wsp_tsp}
    Suppose we have two sets of points $A$ and $B$ that are $s$-well-separated where $s > 1$. Then $A$ and $B$ must be TSP-separated.
\end{corollary}
...
Analyzing the program there are $n-1$ edges in the MST, and for each edge we computer the connected components in $O(n)$ time, compute the diameter of each component in $O(n^2)$ time, and check the separation condition in $O(1)$ time. Thus the overall runtime is $O(n^3)$.

\begin{lem}\label{lem:mst-contains-min-edge}
    Let $(A,B)$ be a bi-partition of a set of points $P$ within a metric space. Then any MST of $P$ must contain an edge which represents the minimum distance between $A$ and $B$.
\end{lem}
\begin{proof}
...
\begin{thm}
    If there exists a bi-partition of the points that is $s$-well-separated, then \cref{alg:balanced-bipartition} will find a bi-partition that is $s$-well-separated and maximally balanced.
\end{thm}
\begin{proof}[Sketch of Proof]
    It follows from \cref{lem:mst-contains-min-edge} that if there exists a bi-partition of the points that is $s$-well-separated, then there must be an edge in the MST that represents the minimum distance between the two clusters in that bi-partition.
    If it were the case that more than one edge needed to be cut from the MST to create the two clusters in the bi-partition, then it would have a separation factor of $s \leq 1$ contradicting the assumption that the bi-partition is $s$-well-separated for $s > 1$ (this argument can be observed by noting the diameter of the outer set must be at least as long as the cut edge).
    Therefore, we can conclude that the algorithm iterates over all applicable bi-partitions of the points and selects the most balanced one among them.
\end{proof}
