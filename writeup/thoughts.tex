...
\begin{lem}
    Let $e = (u,v)$ be an edge in the MST of a set of points $P$ in a metric space. Then removing $e$ from the MST creates two disjoint sets of points $A$ and $B$. The weight of edge $e$ is strictly the minimum distance between any point in $A$ and any point in $B$.
\end{lem}
\begin{proof}
    Assume by contradiction that there exists a edge $e' = (x,y)$ where $x \in A$ and $y \in B$ such that the weight of $e' < e$.
    Then if we add edge $e'$ to the MST we create a cycle. This cycle must cross $A$ and $B$ exactly twice (once at $e'$ and once at $e$).
    Then we can remove edge $e$ from the cycle to get a new spanning tree with weight less than the original MST which is a contradiction.
    Therefore the weight of edge $e$ must be strictly the minimum distance between any point in $A$ and any point in $B$.
\end{proof}

\section{Algorithms}

\begin{algorithm}[H]
\caption{\textsc{FindBestBalancedSeparation}$(D,s)$}
\label{alg:find_best_balanced_separation}
\begin{algorithmic}[1]
\Require A metric distance matrix $D \in \mathbb{R}^{n \times n}$ (symmetric, triangle inequality), separation factor $s > 0$
