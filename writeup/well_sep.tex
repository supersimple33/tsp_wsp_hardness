...
\begin{abstract}
    ...
    Our main results revolves around proving that if a point set can be partitioned into two well-separated sets, then any optimal TSP tour must visit the points in each set consecutively.
    We then show how if the separation holds in a hierarchical manner, then the optimal TSP tour must respect the hierarchical structure of the WSPD, meaning that there is exactly one or two edges connecting the points of each well-separated pair in the decomposition.
    Furthermore, we develop efficient algorithms for testing whether a given point set can be partitioned into well-separated sets, and how to leverage such a partitioning to efficiently compute optimal TSP tours.
